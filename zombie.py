...
class Zombie:
    ...
    def escape_to_boy(self, r=0.5):
        self.state = 'Walk'
        self.move_slightly_to(play_mode.boy.x * -1, play_mode.boy.y * -1)
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('소년보다 공이 많은가?', self.is_boy_more, 7)
        a4 = Action('소년한테 접근', self.move_to_boy)
        a5 = Action('소년에게서 도망', self.escape_to_boy)
        root = SEQ_neardy_boy = Sequence('공의 갯수 비교', c1, c2)

        root = SEQ_chase_boy = Sequence('소년을 추적', SEQ_neardy_boy, a4)

        root = SEQ_escape_boy = Sequence('소년에게서 도망', c1, a5)

        root = SEQ_chase_escape = Selector('추적 또는 도망', SEQ_chase_boy, SEQ_escape_boy)

        a6 = Action('순찰 위치 가져오기', self.get_patrol_location)
        root = SEQ_patrol = Sequence('순찰', a6, a2)

        root = SEL_chase_or_flee = Selector('추적 또는 배회', SEQ_chase_escape, SEQ_wander)
